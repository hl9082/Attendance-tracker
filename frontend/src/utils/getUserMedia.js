...
export const startCamera = async (videoRef, setIsCameraActive) => {
    try {
      ...
      // Log more specific error messages based on the error type
      if (err.name === 'NotFoundError') {
        ...
        alert('The requested camera constraints could not be satisfied. Try adjusting the constraints.');
      } else if (err.name === 'AbortError') {
        alert('The camera request was aborted unexpectedly.');
      } else {
        alert('An unknown error occurred while accessing the camera. Please check your device and browser settings.');
      }
    }
  };
